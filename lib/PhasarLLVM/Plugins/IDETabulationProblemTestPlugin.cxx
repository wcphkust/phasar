...
void IDETabulationProblemTestPlugin::printDataFlowFact(
    std::ostream &os, const FlowFact *d) const {
  auto wrapper = static_cast<const FlowFactWrapper<const llvm::Value *> *>(d);
  os << llvmIRToShortString(wrapper->get());
}

void IDETabulationProblemTestPlugin::printValue(std::ostream &os,
                                                const EdgeFact *v) const {
  auto wrapper = static_cast<const EdgeFactWrapper<const int64_t> *>(v);
  if (wrapper->get() == BOTTOM) {
    os << "Bottom";
  } else if (wrapper->get() == TOP) {
    os << "Top";
  } else {
    os << to_string(wrapper->get());
  }
}
